...
class CommandPipelineTest extends IntegrationTestBase {
    ...
    class CreateTaskTests {
        ...
        void createTask_fullPayload_success() throws Exception {
            ...
            var task = taskRepository.findByHouseholdIdOrderByCreatedAtDesc(testHousehold.getId()).get(0);
        ...
        void createTask_assigneeNotMember_rejected() throws Exception {
            var request = Map.of(
                    ...
                    "payload", Map.of(
                            "title", "Some task",
                            "assigneeId", testUser2.getId().toString() // testUser2 is not a member
                    ),
                    "source", "api"
            );
    ...
    class CompleteTaskTests {
        ...
        void completeTask_success() throws Exception {
            ...
            var createRequest = Map.of(
                    "householdId", testHousehold.getId().toString(),
                    "type", "create_task",
                    "payload", Map.of("title", "Task to complete"),
                    "source", "api"
            );
            ...
            var createResponse = objectMapper.readTree(createResult.getResponse().getContentAsString());
        ...
        void completeTask_notFound_rejected() throws Exception {
            var request = Map.of(
                    "householdId", testHousehold.getId().toString(),
                    "type", "complete_task",
                    "payload", Map.of("taskId", UUID.randomUUID().toString()),
                    "source", "api"
            );
    ...
    class AuthorizationTests {
        ...
        void notHouseholdMember_forbidden() throws Exception {
            var request = Map.of(
                    "householdId", testHousehold.getId().toString(),
                    "type", "create_task",
                    "payload", Map.of("title", "Some task"),
                    "source", "api"
            );
    ...
    class CorrelationIdTests {
        ...
        void providedCorrelationId_used() throws Exception {
            ...
            var request = Map.of(
                    "householdId", testHousehold.getId().toString(),
                    "type", "create_task",
                    "payload", Map.of("title", "Test task"),
                    "source", "api"
            );
